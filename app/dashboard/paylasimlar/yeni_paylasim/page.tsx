...
export default function Paylasimlar() {
    ...
    useEffect(() => {
        async function fetchFacebookLogin() {
            try {
              const response = await fetch('https://meropsybackend-production.up.railway.app/authfacebook/facebook');
              if (!response.ok) {
              ...
              // Örneğin, kullanıcıyı giriş yaptıktan sonra bir sayfaya yönlendirebilirsiniz.
            } catch (error) {
              console.error('Facebook login isteğinde hata oluştu:', error);
            }
          }
          
          // Bu fonksiyonu çağırarak login işlemini başlatın
          fetchFacebookLogin();
      }, []);
            ...
            <div className="flex flex-row">
                <div className="w-1/4">
                    ...
                    <div className="flex flex-row mt-5">
                        <span className="rounded bg-[#7367F0] px-4 py-1">
                            <span className="text-white">Reels</span>
                        </span>

                        <span className="rounded bg-[#20C997] px-4 py-1 mx-5">
